...
- CI `35542701733`, job `106163129812`: unit 224, lint, typecheck, build, database 106, and authentication 4 passed. Both Task 7 projects then timed out selecting the exact localized location-type label after the location-name input was found. The wrapped select could expose its selected option as part of its accessible name; an explicit translated `aria-label` now gives the control a stable exact name without weakening the E2E assertion.
- PR run `35542704255`, job `106163136351`, on the same SHA had one pre-existing authentication failure waiting for `Profil gespeichert.` while three authentication projects passed; push CI on that SHA passed all four authentication projects. This variability is recorded without an unrelated auth change.
