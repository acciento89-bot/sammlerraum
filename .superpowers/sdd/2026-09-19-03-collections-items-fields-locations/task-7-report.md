...
- Batch 1 CI `35545440699`, job `106170379220`, reached 232 passes with one focused failure: the implementation used Intl's valid German default `20.9.2026` while the test unnecessarily required padded `20.09.2026`. The assertion now accepts the locale-native default; production date formatting is unchanged.
- Batch 1 follow-up CI `35545648370`, job `106170933645`, passed all 233 unit tests with 20 database skips. Ordinary lint failed only on the three changed TypeScript files; the failure-only diagnostic emitted exact formatter output for those files, now persisted byte-for-byte. Later gates did not run, so no browser behavior is claimed from this run.
