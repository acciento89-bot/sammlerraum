...
Same-email social sign-in never links an existing account implicitly. A signed-in user must start an explicit provider-linking flow. Direct Better Auth account unlinking and passkey deletion remain disabled so they cannot bypass the verified-recovery-factor rule.
